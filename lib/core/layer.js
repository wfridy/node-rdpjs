...
BufferLayer.prototype.startTLS = function(callback) {
	...
	// Emitted after handshake, must verify the certificate received by the other end
	this.secureSocket.on('secure', (secureErr) => {
		const verifyErr = secureSocket.ssl.verifyError(); // Verify valid certificate
			 // Add this in if we get the host info
			 // || tls.checkServerIdentity(host, this.getPeerCertificate()); // Verify we received cert from the host we were expecting
		log.error({
			secureErr,
			verifyErr
		}, 'problems creating secure connection over socket');
		console.log('SWG secure connection', {
			secureErr,
			verifyErr
		});
		callback(err || verifyErr);
	});
...
BufferLayer.prototype.listenTLS = function(keyFilePath, crtFilePath, callback) {
	...
	this.secureSocket.on('secure', (secureErr) => {
		...
		console.log('SWG secure connection2', {
			secureErr,
			verifyErr
		});
		callback(err || verifyErr);
