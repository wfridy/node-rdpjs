...
const RdpdrChannelDef = new type.Component({
  name : new type.BinaryString(new Buffer('rdpdr' + '\x00\x00\x00', 'binary'), { readLength : new type.CallableValue(8) }),
  options : new type.UInt32Le(0x80800000)
});

const RdpsndChannelDef = new type.Component({
  name : new type.BinaryString(new Buffer('rdpsnd' + '\x00\x00', 'binary'), { readLength : new type.CallableValue(8) }),
...
MCS.prototype.recv = function(s) {
	...
	console.log('[MCS] recv: channelId', channelId);
...
Client.prototype.recvChannelJoinConfirm = function(s) {
    ...
    // console.log(`recvChannelJoinConfirm ${channelId}`);
    this.connectChannels();
};
...
Client.prototype.sendConnectInitial = function() {
	...
	const d = ConnectInitial(ccReq).encode(asn1.ber);
...
Client.prototype.connectChannels = function(s) {
	...
	// console.log('connectChannels', this.channels[this.channelsConnected].id);
