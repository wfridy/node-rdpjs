...
class Client extends Cliprdr {
  ...
  send(message, { first = 0x01, last = 0x02 } = {}, size ) {
    this.transport.send('cliprdr', new type.Component([
      // Channel PDU Header
      new type.UInt32Le(size || message.size()),
      // CHANNEL_FLAG_SHOW_PROTOCOL - 0x10
      new type.UInt32Le(first | last | 0x10),
      message
    ]));
  };
  ...
  sendFormatDataResponsePDU() {
    ...
    // span multiple PDUs
    if (this.content.length > MAX_STR_LENGTH) {

      const bufs = new Buffer(this.content + '\x00' , 'ucs2');
      const bufsLen = bufs.length;
      const size = new type.Component({
        msgType: new type.UInt16Le(data.ClipPDUMsgType.CB_FORMAT_DATA_RESPONSE),
        msgFlags: new type.UInt16Le(0x01),
        dataLen: new type.UInt32Le(bufsLen),
        requestedFormatData: new type.BinaryString(bufs, { readLength : new type.CallableValue(bufs.length)})
      }).size();
      
      // fisrt
      {
        const content = this.content.substr(0, MAX_STR_LENGTH);
        this.content = this.content.substr(MAX_STR_LENGTH);
        const bufs = new Buffer(content, 'ucs2');
        this.send(new type.Component({
          msgType: new type.UInt16Le(data.ClipPDUMsgType.CB_FORMAT_DATA_RESPONSE),
          msgFlags: new type.UInt16Le(0x01),
          dataLen: new type.UInt32Le(bufsLen),
      ...
      while (this.content.length > MAX_STR_LENGTH) {
        const content = this.content.substr(0, MAX_STR_LENGTH);
        this.content = this.content.substr(MAX_STR_LENGTH);
        const bufs = new Buffer(content, 'ucs2');
        this.send(new type.BinaryString(bufs, { readLength : new type.CallableValue(bufs.length)}), { first: 0, last: 0 }, size);
      }

      // last
      {
        const bufs = new Buffer(this.content + '\x00' , 'ucs2');
        this.send(new type.BinaryString(bufs, { readLength : new type.CallableValue(bufs.length)}), { first: 0 }, size);
      }
